...
- Engine code lives under `engine/`: public headers in `engine/include/RTRMetalEngine`, implementation in `engine/src` grouped into `Core`, `Rendering`, `Scene`, and `MPS` bundles.
...
- Run sample: `./build/RTRMetalSample --scene=cornell --frames=16` (accepts resolution/hash flags plus `--mode=auto|hardware|fallback` to pick the ray tracing backend).
  - `hardware` forces Metal RT (logs an error if the TLAS isn't ready), `fallback` uses the gradient diagnostic path.
- Run MPS demo: `./build/RTRMetalMPSSample --gpu --accum-frames=4` for convergence checks.
...
- Align feature work with Apple’s official Metal ray tracing samples (`~/Desktop/metal_RTR_official_example`) and the vetted open-source fallback (`~/Desktop/MetalRayTracing`).
